...
if (isset($_SESSION['login'])) {
  header('Location:http://localhost/nolepverse-native/error/logged');
}
...
<body class="bg-slate-200 dark:bg-slate-800 flex flex-col gap-3 items-center justify-center min-h-screen">
  <?php
    $mix->authToast('username-taken', 'bg-red-400', 'Username already taken!');
    $mix->authToast('email-taken', 'bg-red-400', 'Email already taken!');
    $mix->authToast('password-shorten', 'bg-red-400', 'Password yang anda masukkan harus lebih dari 6 karakter');
    $mix->authToast('password-space', 'bg-red-400', 'Password tidak boleh mengandung spasi');
    $mix->authToast('password-numb', 'bg-red-400', 'Password anda harus mengandung angka');
  ?>
  <div class="bg-slate-50 dark:bg-slate-900 dark:text-white p-4 w-96 shadow-xl">
    ...
    <form action="" method="post">
      ...
      <input type="email" name="email" placeholder="Email" class="bg-slate-200 dark:bg-slate-800 mt-4 w-full py-2 indent-4 rounded-md">
      ...
      <button type="submit" name="btn-register" class="w-full bg-slate-900 dark:bg-green-400 hover:bg-slate-700 hover:dark:bg-green-300 duration-200 text-white py-2 mt-4 mb-8 rounded-md">Create Account</button>
    </form>
    <a href="<?= BASE_URL ?>/auth/login/" class="hover:text-slate-400 duration-200">Have an account?</a>
  </div>
